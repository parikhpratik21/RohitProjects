...
namespace productionmanagment
{
    public partial class Form1 : Form
    {
                ...
                {                   
                    ...
                    if (S == "2")
                    {
                        try
                        {
                            ...
                            sMessage = TARGETLABLE.Text.PadLeft(4, '0') + PLANLABLE.Text.PadLeft(4, '0') + ACTUALLABLE.Text.PadLeft(4, '0') + PDILABLE.Text.PadLeft(4, '0');
                            ba = asen.GetBytes("NKMLINE1:" + sMessage + "\r\n");
        ...
        void readPDIFile()
        {
            filepath = PDIFilePath.Text;
            try
            {
                File.Copy(filepath, Application.StartupPath + "\\data2.txt", true);
            }
            catch
            {

            }
            StreamReader file = new StreamReader(Application.StartupPath + "\\data2.txt");
            try
            {
                string newvalue = file.ReadLine().Split('|')[3].ToString();
                Int64 diff = Convert.ToInt64(newvalue) - Convert.ToInt64(PDILABLE.Text);
                UpdatePDITextValue(newvalue.ToString());
                if (diff != 0)
                {
                    //    ATCUALTOTALLABLE.Text = (Convert.ToInt64(ATCUALTOTALLABLE.Text) + diff).ToString();
                    cmd2_send = true;
        ...
        private void UpdatePDITextValue(string pValue)
        {
            if (PDILABLE.InvokeRequired == true)
            {
                updateActualTextUI updateUI = new updateActualTextUI(UpdateActualTextValue);

                object[] args = new object[1];
                args[0] = pValue;
                PDILABLE.Invoke(updateUI, args);
            }
            else
            {
                PDILABLE.Text = pValue;
            }
        }
            ...
            else
            {
                ...
                if (file_read_counter > 5)
                {
                    ...
                    if (fileavile)
                    {
                       ...
                       Thread m_serverThread = new Thread(() => readActualFile("3"));// new Thread(new  ParameterizedThreadStart(voicecallstart(a)));//soundpipe.voicesoundwrite(modems.Rows[a]["voiceport"].ToString(), Convert.ToInt32(modems.Rows[a]["voicePortSpeed"].ToString()), voicebuffer[a],a);
                        m_serverThread.Start();                        
                    }

                    if(pdifileavile)
                    {
                        Thread m_serverThread1 = new Thread(() => readPDIFile());// new Thread(new  ParameterizedThreadStart(voicecallstart(a)));//soundpipe.voicesoundwrite(modems.Rows[a]["voiceport"].ToString(), Convert.ToInt32(modems.Rows[a]["voicePortSpeed"].ToString()), voicebuffer[a],a);
                        m_serverThread1.Start();
                    }
                }
                string h_date = today.Date.Day.ToString() + "-" + today.Date.Month.ToString() + "-" + today.Date.Year.ToString();
                //DATELABLE.Text = String.Format("{0:dd/MM/yy}", today).ToUpper();
        ...
        public void read_setting()
        {
            ...
            try
            {
                ...
                Holidaygrid.Columns[1].Width = 170;
                Holidaygrid.Columns[2].Width = 152;
            ...
            try
            {
                ...
                PDIFilePath.Text = settings.Rows[0]["COMMODE"].ToString(); 
                ...
                if(File.Exists(PDIFilePath.Text))
                {
                    pdifileavile = true;
                }
        ...
        private void btnSend_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (SCROLLING.Text == "Display")
                {
                    ...
                    ba = asen.GetBytes("NKMLINE2_DISP:" + sDisplayMessage + "\r\n");
                }
                else if (SCROLLING.Text == "Scrolling")
                {
                    ...
                    ba = asen.GetBytes("NKMLINE2_SCRL" + ScrollSpeed.Text + ":" + sDisplayMessage + "\r\n");
                }
                stm.WriteTimeout = 500;
                stm.Write(ba, 0, ba.Length);
                tcpclnt.Close();
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            if (directchoosedlg.ShowDialog() == DialogResult.OK)
            {
                PDIFilePath.Text = directchoosedlg.FileName;
            }
        }               
